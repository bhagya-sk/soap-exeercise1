...
public class StockTradeDaoCSVImpl implements IStockTradeDao {
	...
	public List<StockTrade> getStockTradeRecords()
		...
		try (Stream<String> line = Files.lines(Paths.get(file.toURI()))) {
			...
			stockTrdList = line.skip(1).map(strLine -> {
				...
				} catch (InvalidDBRecordFormatDaoException invalidStockTrdExp) {
			...
			throw new DBAccessFailureDaoException("unable to access the DB " + FILE_NAME + ioExp);
		}
	 ...
	 *                                          invalid.
	 * @throws InvalidDBRecordFormatDaoException when format of the db record is invalid.
	 */
	private StockTrade parseStockCSVLine(String strLine) throws InvalidDBRecordFormatDaoException {
		...
		try {
			...
			throw new InvalidDBRecordFormatDaoException("invalid number format.", numformatexp);
		} catch (ParseException parseExp) {
			LOGGER_OBJ.error("date format of the stocktrade field in "+strLine+" is invalid.");
			throw new InvalidDBRecordFormatDaoException("invalid date format.", parseExp);
		}
	 ...
	 * 
	 * @throws OperationNotSupportedDaoException when the operation is not supported by
	 *                                        the database.
	 */
	public void createStockTradeRecord(StockTrade stockTradeObj) throws OperationNotSupportedDaoException {

		throw new OperationNotSupportedDaoException("creating a record in the database is not supported.");
	}

	/**
	 * Deletes the stockTrade records from the database.
	 * 
	 * @throws OperationNotSupportedDaoException when DB operation is not supported.
	 */
	public void deleteStockTradeRecord(int stockTradeId) throws OperationNotSupportedDaoException {
		throw new OperationNotSupportedDaoException("deleting a record from the database is not supported.");
	}
	 ...
	 */
	@Override
	public void updateStockTradeRecord(int stockTrdId, double volume) throws OperationNotSupportedDaoException {
